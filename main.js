...
const calcular = (evento) => {
    ...
    const quantoFicouFinal = Math.round(quantoFicou)
    const mensalFinal = Math.round(mensal)

    const tarefa = document.querySelector('[data-task]')
    const conteudo = `<p class="content"> R$${quantoFicouFinal},00 </p>`
    tarefa.innerHTML = conteudo

    const tarefa2 = document.querySelector('[data-task2]')
    const conteudo2 = `<p class="content">R$${mensalFinal},00</p`
